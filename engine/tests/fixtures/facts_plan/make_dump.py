...
`.text/` sidecars `extract-attachment` writes (`cache_path`'s own names, each
with its `<name>.sha256` holding the SOURCE file's digest) — describing four
paper forms, one of them filed in a `forms/` subdirectory; and one file
(`.xyz`) nothing can read. A fifth form exists only in `TRANSCRIPT`.
"""
import hashlib
import json
import pathlib
...
# The department's attachments, in the states §3.8 needs: a `.docx` whose text
# was cached, a `.pdf`, an image and a nested `.docx` likewise, and one file
# with no converter. `(name, cached text)`; a `None` text is a file
# `extract-attachment` writes nothing for.
ATTACHMENTS = (
    ("فرم-تحویل-انبار.docx",
    ...
    # one level down: a form the owner filed in a folder of their own
    ("forms/فرم-انبارگردانی.docx",
     "فرم انبارگردانی ماهانه\n"
     "ستون‌ها: نام کالا | موجودی شمارش‌شده (کیلوگرم) | اختلاف\n"
     "فرم‌های پرشده در دفتر انبار بایگانی می‌شود و سرپرست انبار مسئول آن است.\n"),
    ("چیدمان-انبار.xyz", None),
)

# The fifth form: nobody photographed it, and it exists only in what was said.
TRANSCRIPT = (
...
def make_attachments(root, department=DEPARTMENT):
    ...
    would have written for them — `cache_path`'s own name beside its `.sha256`
