...
NOT_SWEPT: dict[tuple[str, str], str] = {
    ...
    ("GET", "/api/departments/{code}/reports/{kind}/file.{ext}"): (
        "**RESOLVED — do not delete this entry without reading D56's Downloads "
        "row.** Both halves of D12 are asked, in D56's order, by the one gate "
        "every other route uses: `routers/reports.py::download_report` depends "
        "on `access.requires('export_pdf', _report_target)`. Scope: the target "
        "is `dept:{code}/report:{kind}`, built from the path alone, and a "
        "caller whose scopes do not reach it gets the same bare 404 a report "
        "nobody has built gets. Capability: downloading is authorised by "
        "`export_pdf` (D25), so a `reader_no_download` holder scoped to the "
        "department is refused **403** — a resource they can see, an action "
        "they may not take — with `access.denied` recorded (D42), and a caller "
        "who fails both halves gets the 404. What was still open here was the "
        "**shared export credential**, which carried no identity and therefore "
        "neither scope nor role, so a caller holding it alone was served every "
        "department and asked for no capability; D24 retired it outright, and "
        "with it `export_auth.py` and `routers/export_files.py`. Pinned end to "
        "end in test_reports_download.py. Excluded from the sweep here because "
        "what this file scans is response bodies for foreign ids, and this "
        "route's body is an opaque file — the authorisation is the whole "
        "question and it is pinned where it lives."),
