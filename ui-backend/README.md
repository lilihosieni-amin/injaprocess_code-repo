...
| `SESSION_SIGNING_KEY` | yes | Required at startup. It signed the export-only session cookie, which is retired (ARD §13.5); the UI session cookie carries an opaque session id and is **not** signed with it. |
...
| `EXPORT_DIR` | no | Directory the built report artifacts are written to, and served from by `GET /api/departments/{code}/reports/{kind}/file.{ext}` — behind the caller's own session and their `export_pdf` capability, never a mount of its own. A pure cache, keyed by content. **Unset = the department export feature is off**: the build answers `503`, the download answers `404` and the UI says exporting is unavailable. In Docker the compose file supplies `/exports` (a named volume, deliberately outside the data-repo). |
