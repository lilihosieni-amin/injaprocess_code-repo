...
`{depts}-{date}`, where `{depts}` is the selected department codes joined with `_`, and `{date}` is the meeting's **Shamsi (Jalali)** date written with Latin digits as `YYYY-MM-DD`; a same-day repeat gets a `-02` (and higher) suffix. Examples: `meetings/audio/cooking-1405-04-19.ogg`, `meetings/audio/cooking_dining-1405-04-19.ogg`. The user enters the date in Shamsi (Persian or Latin digits, `/` or `-`); the upload bot validates it with `jdatetime` and normalises it to canonical Latin `YYYY-MM-DD`. (Machine timestamps in `process.json`/`overview.json` remain Gregorian ISO-8601 UTC.) Generated by the upload bot's deterministic function.
...
- **`comment.addressed` arrives through an outbox** (spec D59). `comments resolve` runs in control-bot, which sees `comments.db` and never `app.db` (§19.1), so it appends to an append-only `outbox` table in the shared file and `ui-backend` drains it into the activity record. Four rules keep §19.1's boundary real: the drain **stamps** `actor = agent:control-bot` and never reads an actor from the payload (the agent can write that file by other means, so a trusted actor field would let it forge rows in the Editor's name); the outbox row's monotonic id becomes a unique key on the activity row, since two SQLite files admit no cross-file transaction and a crash mid-drain would otherwise duplicate; the drained row keeps the outbox row's timestamp, not the drain time; and draining happens on a timer *and* before any activity-report query. Moving the activity table into `comments.db` instead would be simpler and would destroy the boundary entirely.
- **`process.edited` from a pipeline run or chat edit, and `confirmation.invalidated`, are projected from git** (spec D60). `ui-backend` walks `git log` in `data-repo` from a stored sha marker and emits one event per commit touching `departments/**`, taking the actor from the commit (§15's three write paths each carry a distinct identity) and the time from the commit. Routing `merge` through the outbox was rejected: it is a deterministic engine CLI under INV-1/INV-2, used by the pipeline, by chat edits and indirectly by the UI, and giving it a dependency on `comments.db` couples the data engine to the comment system for no reason. The same pass is the **only** writer of `confirmation.invalidated` — nothing "invalidates" a confirmation under §19.6, the fingerprint simply stops matching, and a commit touching the process is exactly what stops it.
