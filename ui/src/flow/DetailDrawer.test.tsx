...
describe('DetailDrawer view', () => {
  it('shows label, actor, description, ICOM chips and the id', () => {
    render(<DetailDrawer node={n} editing={false} conflicts={[]} onClose={() => {}} onEdit={vi.fn()} onAccept={vi.fn()} onReject={vi.fn()} onOpenSub={vi.fn()} onPatch={vi.fn()} onLinkSub={vi.fn()} onSetJunction={vi.fn()} process={{ nodes: [] } as never} onCreateSub={vi.fn()} onDeleteNode={vi.fn()} />)
