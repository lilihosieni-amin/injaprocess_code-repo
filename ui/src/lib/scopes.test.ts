...
import {
  EVERY_DEPARTMENT, NO_DEPARTMENT, parseScope, reportLabel,
...
/** The backend registry (D26), as `GET /api/reports` really answers it today. */
...
describe('scopeLabel', () => {
  it('names `*` as everything, and never as a list of today’s departments', () => {
    ...
    expect(scopeLabel('*', NAMES, REPORTS)).toBe(EVERY_DEPARTMENT)
  })

  it('translates a whole-department scope', () => {
    expect(scopeLabel('dept:dining', NAMES, REPORTS)).toBe('سالن')
  })
  ...
  it('falls back to the stored code when the registry has no name for it', () => {
    ...
    expect(scopeLabel('dept:gone', NAMES, REPORTS)).toBe('gone')
  })
  ...
  it('parenthesises the narrowing, so a joined list reads back as one scope', () => {
    ...
    // back as two scopes.
    expect(scopeLabel('dept:dining/report:flowchart', NAMES, REPORTS)).toMatch(/^سالن \(.*\)$/)
  })
  ...
  it('quotes a scope the grammar refuses, rather than calling it everything', () => {
    for (const refused of ['admin', '', 'dept:Dining', 'dept:dining/report:']) {
      expect(scopeLabel(refused, NAMES, REPORTS), refused).toBe(refused)
      expect(scopeLabel(refused, NAMES, REPORTS), refused).not.toBe(EVERY_DEPARTMENT)
    }
  })
})
...
describe('scopesLabel', () => {
  it('names every scope a person holds, not just the first', () => {
    ...
    expect(scopesLabel(['dept:cashier', 'dept:dining'], NAMES, REPORTS)).toBe('صندوق، سالن')
  })

  it('says so when there are none', () => {
    expect(scopesLabel([], NAMES, REPORTS)).toBe(NO_DEPARTMENT)
  })
})
...
describe('reportLabel', () => {
  it('answers undefined for a kind this build has never heard of', () => {
    ...
    expect(reportLabel('audit', REPORTS)).toBeUndefined()
    for (const kind of Object.keys(REPORTS)) expect(reportLabel(kind, REPORTS)).toBeTypeOf('string')
  })

  it('answers undefined for every kind when the registry has not arrived', () => {
    expect(reportLabel('steps', {})).toBeUndefined()
  })
})
