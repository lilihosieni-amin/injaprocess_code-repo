...
| `0003-preset-append-system-prompt.patch` | v1.6.0 passes `system_prompt` as a plain **string**, which the SDK emits as `--system-prompt` and thereby **replaces** Claude Code's built-in agentic system prompt. Without it the model stops mid-pipeline (right after the Stage-5 extract subagents return) and the bot reports success while the run is frozen at Stage 5 (`meta.json` `finished_at:null`). The patch switches to `{"type":"preset","preset":"claude_code","append": base_prompt}` → `--append-system-prompt`, keeping the default harness **and** our directory/CLAUDE.md guidance. (This bot version is SDK-only; `USE_SDK` is not read by the code.) |
