...
| `0005-raise-production-budget-caps.patch` | `ProductionConfig` (environments.py) hardcodes `claude_max_cost_per_request=2.0` and `claude_max_cost_per_user=5.0`, and `loader._apply_environment_overrides` makes the profile **clobber the env vars** (CLAUDE_MAX_COST_PER_REQUEST=50 / …_USER=100 are ignored). `claude_max_cost_per_request` becomes the SDK `max_budget_usd` **per-turn** cap; a full `/process-voice` turn costs ~$2, so the model gets a `budget_usd` "nearly out" signal and stops mid-extract («بودجهٔ این اجرا در حال اتمام است»), resetting each message → repeated manual "ادامه بده". The patch raises both caps to `100.0` so a full multi-stage run fits in one turn. (Also see deploy/docker-compose.yml: `CLAUDE_MAX_TURNS=300` headroom and `BASH_DEFAULT_TIMEOUT_MS=600000` for slow `transcribe`.) |
