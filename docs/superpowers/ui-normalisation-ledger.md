...
| L-62 | A token the ruling spent | R47 minted `--text-crumb-sep` for the reader's «/» separator at the design's own `#DCD3EC`. R48 then drew that separator in `--text-body` instead, because the design's ink measured 1.44:1 | 1 | **Decided — a consequence of R48, recorded so it is not read as an orphan.** The token stays declared and unconsumed: its element *is* drawn, in another colour, so this is a name the ruling **spent** rather than one still waiting. Its `UNPAINTED_BECAUSE` reason says exactly that, instead of "waiting on a ruling" — the distinction matters, because the close-out sweep's job is to drive that list to zero and an unexplained survivor reads as unfinished work. |


| L-63 | Where a value sits in its grid column | Every one of the four tables built on `FactGrid` start-aligns its head and its cells — the decision table (:1198), the record grid (:1358), the columns table (:1379) and the edge cases (:1660) — and each cell's span is `display:inline-block`, so the CELL places it whatever the span's own `dir` says | 4 of 4 | **Decided — owner ruling (R52): a value sits in the MIDDLE of its column.** «the table content doesn't show correctly … the data should be center of column», 2026-09-06, reported against the units record. Applied to the two grids whose columns hold values — the record grid and the decision table — and deliberately not to the columns table or the edge cases, each of which carries a column of wrapped Persian prose (a field's notes, an edge case's «چرا»), where a centred paragraph is a different thing from a centred value. `FactGrid` takes `align`, defaulting to the design's `start`, so the divergence is opt-in and readable at each call site rather than baked into the shared grid. **The report also uncovered a plain defect underneath the request, fixed separately and not by the centring:** this build had rewritten the design's `inline-block` span as `block truncate`, and a block box makes its own alignment context — `text-align: start` then resolved against the span's OWN `dir`, sending every `dir="ltr"` island (a symbol, a number) to the left of its column while every Persian cell stayed right. `CELL_TRUNCATE` restores the design's idiom, which truncates identically and cannot depend on direction. Centring HIDES that defect (a block child inherits `text-align: center` and centres its text anyway), so it is pinned on the columns table, which stays start-aligned |

| L-64 | What paints a grid's head band and its row rules | The row. Every `FactGrid` row in the design is `display:grid` with `min-width:100%`, carrying the head's `#F8F4FE` and each row's `border-bottom` on the row element itself (:1349, :1356, :1374, :1378) | 4 of 4 | **Decided — diverged, because the design's own markup cannot render a wide table.** A row's tracks are `minmax(110px,1fr)`, so a grid with more columns than fit overflows its box; `min-width:100%` sizes that box to the scroll container and never to the tracks, and background and border paint the box. Past the container's edge both simply stop — reported by the owner on «مواد اولیه پیتزا امریکایی», 2026-09-06: «Starting from a certain column onward, the table has neither a colored header nor the lines drawn between rows.» The design's mock carries no table wide enough to show it, which is why it was drawn this way and why conforming to it here would be conforming to a defect. **Painting moves to the cells, which ARE the tracks**: the covered area becomes exactly right and no layout moves. The grid has no `gap`, so per-cell rules read as one line. The alternative — widening the row with `width:max-content` — was implemented, measured and rejected: it re-sizes every track through the `1fr`, which equalises them all to the widest column, taking `140 + 10×110` to `11×140` and the table 300px wider, for a bug about paint |

| L-65 | The confirm tick on a red entry | A fourth tick state: the design's `sfCanTick` (:5062) carries `red_disputed` and `red_unknown`, drawn through the translucent palette of :5065-5069 as a DISABLED control labelled «قابل تأیید نیست» | 1 of 4 states | **Decided — owner ruling (R53): a red entry is confirmable like any other.** «each of the quantitative items should be confirmable, regardless of whether it has an issue or not», 2026-09-06. This overturns QF-25's red-over-green, which is a spec rule and not only a drawn state, so three things moved together: `can_confirm` stops subtracting red, `POST /api/confirmations/{fid}` drops its 409, and the disabled state is deleted rather than left unreachable. The argument for the rule was that a confirmation should not vouch for an unreconciled fact; against it, and decisive, is that an `unknown` leaf is a question for the SOURCE — nothing a reviewer does on that screen can answer it, so the refusal withheld the signature without moving the thing it was waiting on, on exactly the entries most in need of a reviewer. The red marks stay drawn beside the tick and say the rest, and `red_paths` is asserted beside `can_confirm` so a later change cannot clear the red to earn the tick. Two tokens and one label die with the state: `PX.tickRedChip`, `PX.tickRedOuter` and `BADGE_LABELS.cannot_confirm` |
