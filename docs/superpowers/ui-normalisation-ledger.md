...
| L-23 | Close button | `32×32` radius 8 (drawer), `32×32` radius 9 (modal), `28×28` radius 8 (`DetailDrawer`), `34×34` radius 10 with a 19px glyph (new-user dialog) | 4 | **Owner veto.** Provisionally one close button: `32×32`, `--tile-v2` fill, radius `9px` (`--role-radius-close`), glyph `×` at `18px` in `--text-muted`. The box is settled — `32×32` is 2 of 4 — but **the radius is not, and dominance points the other way**: `8px` is 2 of 4 and `9px` is 1 of 4, and §4.4's own role map states *two* roles, `8px` for the drawer close and `9px` for the dialog close. Collapsing them to one radius is an invention, and `8px` has no token where `9px` is `--radius-sm`. Neither rule 1 nor rule 2 settles it. |
...
| L-40 | Control and input radius | Three heavily-used steps for one family of controls: `12px` (primary button, input, card-in-a-drawer, tab tray, dialog action), `11px` (secondary/ghost button, menu row, tile, back button, small input), `10px` (control, menu item, actor pill, pager button, tab pill). The token set names all three — `--radius-md`, `--radius-input`, `--radius-control` | 72 vs 38 vs 23 | **Owner veto.** Provisionally `12px` for both `--role-radius-control` and `--role-radius-input`, which normalises **61 sites** — the largest single visual change in the role table. It is referred rather than decided because neither rule reaches it: §4.4's role map assigns the three steps to *different* components consistently, so this is not the design contradicting itself, and dominance only decides a tie between variants of one role. If the owner keeps the ladder, the two tokens gain roles of their own and no value moves; if the owner takes the normalisation, 11 and 10 disappear from the product. |
...
Seven rows above, plus three items that are not contradictions and cannot be
...
1. **L-05 — the “awaiting” amber.** `#B4690E` was chosen against the dominant
   ...
   meaning. **Weigh that counter-argument knowing it is already broken:** three of
   the four ICOM hues are shared with app-wide roles — output green `#1F8A5B` is
   accept/approve 38 times, mechanism violet `#4A25A9` is primary 185 times, input
   blue `#1F6FB2` is `--info`. `#8A5A00` is the only ICOM hue the rest of the
   product never touches, so the rule the row invokes is one the design itself
   keeps only once out of four.
...
3. **L-12 — `ProcessTag.plain`.** Whether a plain process shows no tag at all, or
   a muted one.
4. **L-23 — the close button's radius.** The box is settled at `32×32`; the radius
   is not. Dominance says `8px` (2 of 4) and the row provisionally takes `9px`
   (1 of 4) because it is the only one of the two on the token scale — and §4.4
   states two roles, `8px` for a drawer close and `9px` for a dialog close, which
   the row collapses into one. One close button or two?
5. **L-29 — the 4-up stat value.** `23px` centred or `21px` start-aligned. An
   exact four-to-four tie with no stated reason on either side.
6. **L-34 — the reader's screen-title scale.** Kept as R3 wrote it, but its hero
   is smaller than its titles.
7. **L-40 — the control and input radius.** The largest normalisation in the role
   table: `11px` (38 sites) and `10px` (23 sites) collapse into `12px`. The design
   is *consistent* about the ladder — ghost buttons and menu rows at 11, controls
   and pagers at 10, primary buttons and inputs at 12 — so this is a
   simplification the owner is being offered, not a contradiction being resolved.
   Keeping the ladder costs two more roles and no rework.
8. **The Users screen has no subtitle.** Every one of the thirteen panel screens
...
9. **The department card's heavier rest shadow.** The one feature card carries
...
10. **The two z-index values in `ui/src/flow/**` that the adopted scale cannot
