...
- Produces:
  ...
  - `ladder.merge_home(existing, incoming) -> str | None`: when both have a home and they differ, the stored one stays, the entry gains the issue `{"kind": "placement", "description": PLACEMENT_FA, "affects": []}` and the function returns the incoming record id; when the stored entry has no home and the incoming has one, it is adopted (`union`) and `None` is returned.
