...
**On row 5's second half.** «The client filters nothing on visibility» is a
*consequence* of note 5 rather than a clause of it — spec `:1713-1714` says only
that an admin's list is `may_serve`-filtered — and it has **no pin of its own**:
no test fails if a client-side visibility predicate is added above the query.
It is stated here as unpinned rather than carried in the table as though the two
pytest rows covered it. (`FactsList.tsx`'s docstring gives the reason there is
nothing for such a predicate to read: the facts gate is capability-only with no
scope in it.)
...
|---|---|---|
| `ui/e2e/facts.spec.ts` — `column-gap: 7px` on the status-dot row (`:1059`) | the one untokenised value on the list that nothing pinned, while its sibling (the `fr` ratios) was asserted | `style={{ gap: '7px' }}` → `gap-s3` → red, `7px` → **`6px`**: the exact substitution the code comment warns about |
