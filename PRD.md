...
- **FR-P4 (three human approval gates):** **First**, before the set is read, the user **confirms the set of recordings** to be processed — they may have left one out or included an extra — and the system discloses what an explicit list leaves out. **Then**, after the system has read the set and proposed the process set (including its new/update status and any merge, split, or attach; see FR-M1), the user confirms or corrects it (e.g. a process was missed, or two should be merged). On a correction, only this identification is redone, and neither gate has touched any output yet. **Finally**, after the run has been written, the system reviews the whole department and proposes tidying up what earlier runs over-cut or duplicated; the user approves or rejects each proposal individually. Unlike the first two, this gate acts on real output, so approving an item authorises its changes — including replacing values that were already filled (FR-M3).
...
- **FR-M3 (conflict policy):** Empty fields and new items are filled automatically; but where the voice would change a value that is **already filled** (perhaps the user's manual edit), instead of overwriting, that change is recorded as a "pending proposal" and the original value is left untouched. Two paths are exempt because a human has already approved that specific change: a later session that explicitly supersedes an earlier one, and an item approved at the third gate (FR-P4). Everything else goes through the proposal. INV-5 is the rule underneath — existing values are never changed without human approval; the proposal is how approval is obtained when it has not been given yet.
...
- **FR-A1a (the username is a mobile number):** Everyone signs in with their mobile number, in the form `09123456789` and no other. Staff cannot forget it and no two people can share one. Numbers typed with Persian digits, with spaces or dashes, or with a country code are understood and stored in the one canonical form, so the same person cannot end up with two accounts. A number stays attached to its account even after that account is disabled, so it can never be quietly handed to someone else. Note that the system does not send anything to the number or check that the person holds it — it is a name to sign in with, nothing more.
- **FR-A1b (password rule):** At least six characters. Nothing else — no required mixture of characters, no expiry, no ban on reuse. The people using this system work in a kitchen and on a floor, and every extra rule buys less than it costs in passwords written on paper. Signing in behaves identically whether or not the number belongs to a real account, so nobody can learn who works here by trying numbers.
...
- **FR-A5 (roles are fixed; only users are created):** The set of roles is established when the system is set up. **Nobody — including the editor — can create, change or remove a role from inside the system.** A role can never be removed at all, since every user holds one and removing it would leave them with nothing. Adding a further role is a change to the system itself, made by the developer, not a form somebody fills in. Editing, confirming and setting the content-visibility policy are additionally marked as permissions no role may ever be built around, so that the guarantee survives even if role editing were introduced one day.
- **FR-A6 (no exceptions for individuals):** There is no way to add or remove a single permission for a single person. An exception is a further role, added as in FR-A5, and then given to whoever needs it — so taking away one person's ability to download means giving them a role without it, not flipping a switch on their account.
...
- **FR-V7 (the department information page is shown in full):** FR-V5 governs the content of *processes*. A department's own information — what it does, its sub-units, who works there, each role's duties and what each role is measured on — is shown **complete**, with nothing held back and no switches. Two things still decide whether someone sees it at all, and neither is about fields: they must have access to that department, and the editor must have confirmed it (FR-V4). Should a reason to hold part of it back ever arise, it joins the list in FR-V5 rather than becoming a separate mechanism.
...
- **FR-K2a (the editor receives comments and does not write them):** The editor is where the chain ends, so writing a comment there would mean sending it to oneself. They are offered no way to start one, and only ever see comments that have reached them.
...
- **NFR-3 (authentication):** Every user signs in with their own username and password. Passwords are stored securely, never in plaintext. A user can always change their own; someone who manages users can set the password of people they manage (FR-A1), and every such change is recorded against them. Sessions can be ended by the system at once — disabling a user or reducing their access takes effect immediately, not whenever their session happens to expire.
...
- **NFR-12 (nothing carries more than it shows):** What the system hands to a person carries the content they are permitted to see and nothing else — no internal bookkeeping, no unresolved proposals, no record of which recording something came from. This holds for what appears on screen as much as for a downloaded file: **what is withheld is never sent and then hidden.** It applies to whole items as much as to fields: a process that is unconfirmed or retired, a department outside someone's access, another person's comment — none of it is delivered and suppressed by the screen; it is never delivered. Nor is anything that would betray it indirectly, such as a count of things a person cannot read. Asking for something one is not allowed to reach is answered as though it does not exist, so that what exists cannot be mapped by asking.
...
- **AC-25:** Everything a person is not entitled to is absent from what they are sent, not hidden by the screen. Inspecting what the system delivered to a reader — by any means available in a browser — turns up no withheld field, no unconfirmed or retired process, no department they cannot reach, no comment outside their own chain, and no count that would reveal any of these. Asking directly for something outside one's access is answered as though it does not exist.
